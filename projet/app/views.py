...
def index(request):
 if request.method=='GET': #ce code pour afficher les statistique de l'annee universitaire courante automatiquemene
  ...
  for x in y:
    ...
    z=stage.objects.filter(entreprise=x.nom,type='PFE',annee=year).count() # le nombre des entreprise ayant recu des stagiaires de PFE de l'annee courante
    w=stage.objects.filter(entreprise=x.nom,annee=year).annotate(Count('stagiaires')).count() # compter le nombre d'organisme ayant recu des stagiares de stage type PFE pour chaque annee universitraire
    ...
    if year: #si year n'est pass vide on execute le meme code precedent mais selon l'annee entre par lutilisateur
        ...
        for x in y:
          labels.append(x.nom)
          z=stage.objects.filter(entreprise=x.nom,type='PFE',annee=year).count()
          w=stage.objects.filter(entreprise=x.nom,annee=year).annotate(Count('stagiaires')).count()
          data.append(z)
          data2.append(w)
        for an in labels3 :
         a=stage.objects.filter(type='PFE',annee=an).annotate(Count('entreprise')).count()
